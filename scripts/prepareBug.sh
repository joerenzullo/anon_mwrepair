...
# 6th param is the folder where the java 7 installation is located
# 7th param is the folder where the java 8 installation is located
...
if [ "$#" -lt 12 ]; then
  ...
  echo "7th param is the folder where the java 7 installation is located"
  echo "8th param is the folder where the java 8 installation is located"
  ...
  echo "13th param is the oracle genome"
  exit 0
fi
...
./createConfigFile.sh $LOWERCASEPACKAGE $BUGNUMBER $BUGSFOLDER $APPROACH $DIROFJAVA7 $SRCFOLDER $CONFIGLIBS $WD $TESTCP $COMPILECP "$ORACLE_GENOME"
